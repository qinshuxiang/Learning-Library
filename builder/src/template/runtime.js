...
(function () {
  ...
  // 首页固定打开的篇目（AGENTS/AGENTS.md）；该路径不存在时回退到侧栏第一篇
  var DEFAULT_DOC = 'AGENTS/AGENTS.md';
  ...
  function postProcess(path) {
    ...
    // 3) 任务列表复选框（规范自检清单）。marked 在 gfm 下会原生产出
  ...
  function start() {
    ...
    var docToOpen = init || (DOCS[DEFAULT_DOC] ? DEFAULT_DOC : firstDoc(TREE));
