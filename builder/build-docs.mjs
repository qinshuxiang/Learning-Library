 ...
 *
 * 扫描仓库根的 前端/后端/架构/法律 四个领域文件夹与 AGENTS/ 规范目录，将全部 md
...
// 顶层顺序：AGENTS 规范目录排侧栏最前，四个领域文件夹依次随后
const TOP_ORDER = ['AGENTS', '前端', '后端', '架构', '法律'];
const SCAN_DIRS = ['AGENTS', '前端', '后端', '架构', '法律']; // 仓库根下递归扫描的目录
const ROOT_FILES = []; // 仓库根下的单篇文档（不递归扫描），渲染在侧栏根层级；规范已收进 AGENTS/
