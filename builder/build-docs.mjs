 ...
 *
 * 扫描仓库根的 前端/后端/架构/法律 四个领域文件夹与 规范/ 规范目录，将全部 md
 * 原始内容与文件树内嵌进单个自包含 HTML（marked + highlight.js
 * 也一并内嵌），产物双击 file:// 即用。
 *
 * 无副作用：只读 Markdown 与 assets 图片，不在磁盘上生成或改写任何文件，
 * 唯一输出是仓库根的 学习库.html。首页默认打开侧栏第一篇。
 *
 * 用法：本脚本位于 builder/ 子目录，构建命令：cd builder && npm run build
 */
import { promises as fs } from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';

const ROOT = path.dirname(fileURLToPath(import.meta.url)); // builder/ 目录（node_modules、src/template 所在）
const PROJECT = path.resolve(ROOT, '..'); // 仓库根
const OUT = path.join(PROJECT, '学习库.html');

/* ---------------- 顶层领域与优先级 ---------------- */
// 页面逻辑路径相对仓库根
const DOC_ROOT = PROJECT;
// 顶层顺序：规范 目录排侧栏最前，四个领域文件夹依次随后
const TOP_ORDER = ['规范', '前端', '后端', '架构', '法律'];
const SCAN_DIRS = ['规范', '前端', '后端', '架构', '法律']; // 仓库根下递归扫描的目录
const ROOT_FILES = []; // 仓库根下的单篇文档（不递归扫描），渲染在侧栏根层级；规范已收进 规范/
const META_DIRS = ['规范']; // 元文档目录及其全部子目录：只放规范文件、不配 0. 索引，豁免缺索引检查
...
async function buildData() {
  ...
  for (const name of TOP_ORDER) {
    if (ROOT_FILES.includes(name)) {
      const abs = path.join(DOC_ROOT, name);
