 ...
 *
 * 扫描仓库根的 前端/后端/工具/常识/架构/法律 六个领域文件夹与根层规范文件（文档规范.md、
 * 编程规范.md），将全部 md 原始内容与文件树内嵌进单个自包含 HTML（marked + highlight.js
 * 也一并内嵌），产物双击 file:// 即用。
 *
...
const DOC_ROOT = '.';
// 顶层顺序：仓库根的规范文件排在侧栏最前，六个领域文件夹依次随后
const TOP_ORDER = ['文档规范.md', '编程规范.md', '前端', '后端', '工具', '常识', '架构', '法律'];
const SCAN_DIRS = ['前端', '后端', '工具', '常识', '架构', '法律']; // 仓库根下递归扫描的目录
const ROOT_FILES = ['文档规范.md', '编程规范.md']; // 仓库根下的单篇文档（不递归扫描），渲染在侧栏根层级
